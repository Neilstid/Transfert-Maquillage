...
def bulid_delta_inverse(target_control_points):
    ...
    forward_kernel = torch.full((N + 3, N + 3), 0).float().to(target_control_points.device)
    ...
    # Replace all 0 by small values to avoid inversion error
    torch.diagonal(forward_kernel)[torch.diagonal(forward_kernel) == 0] = 1e-10
    # compute inverse matrix
    inverse_kernel = torch.inverse(forward_kernel.float())

...
from .modules.module_base import ResidualBlock_IN, Downsample, Upsample, PositionalEmbedding, MergeBlock, DownsampleAvgPool
...
class Generator(nn.ModuleDict):
    ...
    def __init__(
        self, conv_dim=64, image_size=256, num_layer_e=2, num_layer_d=1,
        window_size=16, use_ff=False, merge_mode='conv', num_head=1,
        double_encoder=False, double_decoder=True, **unused
    ):
        ...
        for i in range(2):
            ...
            # else:
            #     layers = DownsampleAvgPool(curr_dim, curr_dim * 2, affine=True)
        ...
        curr_dim_double_decoder = curr_dim
        ...
        if self.double_decoder:
            curr_dim = curr_dim_double_decoder
            # Bottleneck & Up-Sampling & Merge
            for i in range(2):
                self.add_module('dd_bottleneck_{:d}'.format(i+1), 
                    nn.Sequential(*[ResidualBlock_IN(curr_dim, curr_dim, affine=True) for j in range(num_layer_d)])
                )            
                layers = Upsample(curr_dim, curr_dim // 2, affine=True)
                self.add_module('dd_up_{:d}'.format(i+1), layers)
                curr_dim = curr_dim // 2
                if i < 1:
                    self.add_module('dd_merge_{:d}'.format(i+1), MergeBlock(merge_mode, curr_dim))
            ...
            self.add_module('dd_out_conv', layers)
    ...
    def decode(self, fea_c_list, attn_out_list, unmakeup=False):
        if self.double_decoder and unmakeup:
            for i in range(2): 
                fea_c_ = self['attention_apply_{:d}'.format(i+1)](fea_c_list[i], attn_out_list[i])
                fea_c_ = self['dd_bottleneck_{:d}'.format(2-i)](fea_c_)
                fea_c_list[i] = fea_c_

            # up-sampling & merge
            fea_c = fea_c_list[1]
            for i in range(2):
                fea_c = self['dd_up_{:d}'.format(i+1)](fea_c)
                if i < 1:  
                    fea_c = self['dd_merge_{:d}'.format(i+1)](fea_c_list[0], fea_c)

            fea_c = self['dd_out_conv'](fea_c)
        else:
            # apply
            for i in range(2): 
                fea_c_ = self['attention_apply_{:d}'.format(i+1)](fea_c_list[i], attn_out_list[i])
    ...
    def forward(self, c, s, mask_c, mask_s, diff_c, diff_s, lms_c, lms_s, unmakeup=False):
        """
        ...
        # min_ind = 110
        # max_ind = 120
        # size = max_ind - min_ind

        # transfer_input_s[0][0][0][min_ind:max_ind] *= (torch.rand(size,128,128, device="cuda:0") * 10) - 5
        # (torch.rand(size,128,128, device="cuda:0") * 10) - 5
        # torch.rand(size,128,128, device="cuda:0") * (transfer_input_c[0][0][0][min_ind:max_ind].max() + transfer_input_c[0][0][0][min_ind:max_ind].min()) - transfer_input_c[0][0][0][min_ind:max_ind].min()

        # torch.save(transfer_input_c, r"C:\Users\Neil\OneDrive - Professional\Documents\Python scripts\EleGANt-main\content.pt")
        # torch.save(transfer_input_s, r"C:\Users\Neil\OneDrive - Professional\Documents\Python scripts\EleGANt-main\style.pt")
    ...
    def from_tensor(self, transfer_input_c, transfer_input_s):
        attn_out_list = self.get_transfer_output(*transfer_input_c, *transfer_input_s)
        fea_c = self.decode(transfer_input_c[0], attn_out_list)
        return fea_c

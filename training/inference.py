...
class Inference:
    ...
    """
    def __init__(self, config, args, model_path="G.pth", collab_path="C.pth"):

        self.device = args.device
        self.solver = Solver(config, vars(args), inference=model_path, collab_path=collab_path)
    ...
    def transfer(self, source: Image, reference: Image, postprocess=False, unmakeup=False):
        """
        Args:
        ...
        source_input = self.prepare_input(*source_input)
        reference_input = self.prepare_input(*reference_input)
        result = self.solver.test(*source_input, *reference_input, unmakeup=unmakeup)
    ...
    def transfer_pgt(self, source: Image, reference: Image, postprocess=False, unmakeup=False):
        """
        Args:
            source (Image): The image where makeup will be transfered to.
            reference (Image): Image containing targeted makeup.
        ...
        source_input = self.prepare_input(*source_input)
        reference_input = self.prepare_input(*reference_input)
        result = self.solver.test_pgt(*source_input, *reference_input, unmakeup=unmakeup)
    ...
    def transfer_alpha(self, source: Image, reference: Image, postprocess=False, unmakeup=False):
        """
        Args:
            source (Image): The image where makeup will be transfered to.
            reference (Image): Image containing targeted makeup.
        ...
        result = self.solver.test_alpha(*source_input, *reference_input, unmakeup=unmakeup)

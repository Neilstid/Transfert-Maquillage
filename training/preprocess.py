...
class PreProcess:
    ...
    def __init__(self, config, need_parser=True, device='cpu'):
        ...
        self.nose_class = config["PREPROCESS"]["NOSE_CLASS"]
    ...
    def mask_process(self, mask: torch.Tensor):
        ...
        mask_neck = (mask == self.neck_class[0]).float()
        mask_nose = (mask == self.nose_class[0]).float()

        #mask_list = [mask_lip, mask_face, mask_eyebrow_left, mask_eyebrow_right, mask_eye_left, mask_eye_right]
        mask_list = [mask_lip, mask_face, mask_eye_left, mask_eye_right, mask_neck, mask_nose]
    ...
    def preprocess(self, image: Image, is_crop=True):
        ...
        # cv2.imshow("test", cv2.cvtColor(np_image, cv2.COLOR_RGB2BGR))
        # cv2.waitKey()
    ...
    def lms_resize(self, lms: torch.Tensor, image: Image):
        lms = lms.type(torch.FloatTensor)
        lms *= self.img_size / image.width
        return lms.type(torch.IntTensor)

    def process(self, image: Image, mask: torch.Tensor, lms: torch.Tensor):
        lms = self.lms_resize(lms, image)
